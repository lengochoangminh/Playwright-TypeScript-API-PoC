...
export async function createFutureBooking(roomId: number) {
  ...
  await expect(async () => {
    ...
    const checkOutString = stringDateByDays(
      futureCheckinDate,
      randBookingLength
    );
 ...
 */
export async function getBookingSummary(roomId: number) {
  const createRequestContext = await request.newContext();
  const response = await createRequestContext.get(
    url + `booking/summary?roomid=${roomId}`
  );
...
export async function futureOpenCheckinDate(roomId: number) {
  ...
  for (let i = 0; i < (await currentBookings.bookings.length); i++) {
    ...
    const checkOut = new Date(
      currentBookings.bookings[i].bookingdates.checkout
    );
